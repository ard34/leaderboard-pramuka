...
-- Script ini:
-- 1. TIDAK membuat peserta baru.
-- 2. Memperbarui kolom 'nilai' dan 'rubrik' (ketepatan & waktu resmi) pada data peserta yang ada.
-- 3. Menghapus waktu-waktu tiruan/otomatis sebelumnya sehingga 100% sinkron.
-- =========================================================================

-- 1. Pastikan kolom rubrik JSONB tersedia
...
DECLARE
  ...
  v_lid UUID;
BEGIN
  -- 2. Dapatkan ID Lomba Semaphore di database
  ...
  -- 3. Dapatkan Juri Semaphore atau Admin resmi
  ...
  -- 4. Tabel data resmi Semaphore LT-II 2026
  ...
  ('SMP NEGERI 2 MEKAR BARU', 'SMP', 'Laki-laki', 6, '00:01:46.52'),
  ...
  ('SMP NEGERI 2 MEKAR BARU', 'SMP', 'Perempuan', 2, '00:01:30.99'),
  ...
  FOR r IN SELECT * FROM temp_semaphore_scores LOOP
    ...
    IF v_pid IS NOT NULL THEN
      ...
      SET nilai = r.nilai,
          ...
          updated_at = NOW()
      WHERE peserta_id = v_pid AND lomba_id = v_lid;
