...
export async function POST(request) {
  try {
    ...
    const { peserta_id, reason } = body;

    if (!peserta_id || !reason) {
      return NextResponse.json(
        { error: "Parameter peserta_id dan reason wajib diisi." },
        { status: 400 }
      );
    }
    ...
    // 2. Update participant to rejected
    const payload = {
      is_verified: false,
      catatan_berkas: "DITOLAK: " + reason
    };
      ...
      try {
        ...
        const htmlBody = `
          <div style="font-family: 'Segoe UI', Arial, sans-serif; background-color: #0b1329; color: #f8fafc; padding: 30px; border-radius: 16px; max-width: 600px; margin: 0 auto; border: 1px solid rgba(245, 166, 35, 0.3);">
            ...
            <div style="background: rgba(239, 68, 68, 0.15); border: 1px solid rgba(239, 68, 68, 0.4); padding: 16px; border-radius: 12px; text-align: center; margin-bottom: 24px;">
              <span style="color: #f87171; font-size: 14px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px;">
                ❌ STATUS: DITOLAK
              </span>
            </div>

            <p style="font-size: 14px; line-height: 1.6; color: #e2e8f0;">
              Salam Pramuka! Panitia LT-II Kwartir Ranting Mekar Baru menerangkan bahwa dokumen pendaftaran regu Anda <strong>BELUM LENGKAP / DITOLAK</strong>.
            </p>

            <!-- ALASAN PENOLAKAN -->
            <div style="text-align: left; margin: 28px 0; background: linear-gradient(135deg, rgba(239, 68, 68, 0.1) 0%, rgba(239, 68, 68, 0.05) 100%); border: 2px solid #ef4444; padding: 20px; border-radius: 16px; box-shadow: 0 0 25px rgba(239, 68, 68, 0.15);">
              <div style="font-size: 14px; font-weight: 900; color: #f87171; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 12px;">
                ALASAN PENOLAKAN:
              </div>
            ...
            <div style="background: rgba(255,255,255,0.03); border-radius: 12px; padding: 16px; margin-bottom: 24px;">
              <h3 style="color: #f87171; font-size: 14px; margin-top: 0; margin-bottom: 12px; border-bottom: 1px solid rgba(248, 113, 113, 0.2); padding-bottom: 8px;">KETERANGAN PESERTA</h3>
